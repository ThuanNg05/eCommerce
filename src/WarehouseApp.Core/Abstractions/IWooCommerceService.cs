...
public interface IWooCommerceService
{
    ...
    Task<WooCommerceCategorySyncResult> SyncCategoriesAsync(CancellationToken ct = default);
    Task<WooCommerceProductLinkDto?> GetProductLinkAsync(long productId, CancellationToken ct = default);
    Task<WooCommerceCategoryLinkDto?> GetCategoryLinkAsync(long categoryId, CancellationToken ct = default);
    ...
    Task<WooCommerceCategoryLinkDto> PublishAndLinkCategoryAsync(LinkWarehouseCategoryRequest request, CancellationToken ct = default);
    /// <summary>Pushes the category name when the category is already linked.</summary>
    Task<bool> SyncLinkedCategoryAsync(long categoryId, CancellationToken ct = default);
    ...
    Task<IReadOnlyList<WooCommerceOrderStatusReasonDto>> ListOrderStatusReasonsAsync(string? targetStatus, CancellationToken ct = default);
    Task<bool> AcceptWebhookAsync(string? signature, string? topic, string? eventName, ReadOnlyMemory<byte> payload, CancellationToken ct = default);
}
